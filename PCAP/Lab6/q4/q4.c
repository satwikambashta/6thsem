...
int main(void)
{
	...
	cl_mem c_mem_obj = clCreateBuffer(context, CL_MEM_WRITE_ONLY,LIST_SIZE *sizeof(char), NULL, &ret);
	...
	char *C = (char*)malloc(sizeof(char)*LIST_SIZE);

	ret = clEnqueueReadBuffer(command_queue, c_mem_obj, CL_TRUE, 0,LIST_SIZE*sizeof(char), C, 0, NULL, NULL);
