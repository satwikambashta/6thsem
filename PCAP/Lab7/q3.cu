// write cuda program which performs convolution operation on one dimensional input array N of size width using
// a mask array M of size mask_width to produce the resultant one d array P of size width.
